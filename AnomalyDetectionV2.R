...
trainGAPGRP <- trainDataset[,c("Global_active_power","Global_reactive_power")]
trainGAPGRP <- trainGAPGRP[complete.cases(trainGAPGRP),] #remove NA valued rows
trainFormattedData <- formatMhsmm(data.frame(trainGAPGRP))

# do the same thing for Test data
trainDataset <- read_csv("/home/heather/Code/cmpt-318/test1.txt")
trainGAPGRP <- trainDataset[,c("Global_active_power","Global_reactive_power")]
trainGAPGRP <- trainGAPGRP[complete.cases(trainGAPGRP),]
trainFormattedData <- formatMhsmm(data.frame(trainGAPGRP))
...
kgap <- kmeans(trainFormattedData,5,iter.max=10)
muinit <- kgap$centers
