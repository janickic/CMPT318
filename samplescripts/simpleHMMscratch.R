...
#loading data
dataset <- read_csv("/home/heather/Code/cmpt-318/train.txt")
gap <- dataset[,c("Global_active_power","Global_reactive_power")] #data.frame(dataset$Global_active_power,dataset$Global_reactive_power)

gap <- gap[complete.cases(gap),]



kgap <- kmeans(gap,5,iter.max=10)
muinit <- kgap$centers
muinit
varinit = numeric(5)
kgap$withinss
...
b <- list(mu = muinit, sigma = varinit) 
